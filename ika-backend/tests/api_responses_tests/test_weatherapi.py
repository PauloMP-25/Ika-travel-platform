...
with open("tests/api_responses_tests/weatherapi_response.md", "w", encoding="utf-8") as f:
    f.write("# Pruebas de API: WeatherAPI\n\n")
    f.write(f"**Fecha de ejecución:** {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    
    if not API_KEY:
        f.write("> **Error:** No se encontró la variable de entorno WEATHERAPI_API_KEY. Configúrala antes de ejecutar el script.\n")
    else:
